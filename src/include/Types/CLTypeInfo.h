...
inline void to_json(nlohmann::json& j, const CLTypeInfo& p) {
  ...
  /*
  using CLRVA =
      rva::variant<CLTypeEnum, std::vector<rva::self_t>,
                   CLMap<rva::self_t, rva::self_t>, std::tuple<rva::self_t>,
                   std::tuple<rva::self_t, rva::self_t>,
                   std::tuple<rva::self_t, rva::self_t, rva::self_t> >;

  CLMap<CLRVA, CLRVA> clm1;
  clm1.key = CLTypeEnum::U512;

  std::tuple<CLRVA, CLRVA> t2_1;
  std::vector<CLRVA> v3;

  CLTypeEnum bool_enum = CLTypeEnum::Bool;
  v3.push_back(bool_enum);
  t2_1 = std::make_tuple(CLTypeEnum::U512, v3);
