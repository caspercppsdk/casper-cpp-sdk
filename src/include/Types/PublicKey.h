...
public:
    ...
    static Casper::PublicKey FromBytes(const CBytes& bytes)
    {
        ...
        std::copy(bytes.begin() + 1, bytes.begin() + expectedPublicKeySize, rawBytes.begin());

        return Casper::PublicKey(rawBytes, algoIdent == 0x01 ? KeyAlgo::ED25519 : KeyAlgo::SECP256K1);
    }
    ...
    /// identifier.
    /// </summary>
    static Casper::PublicKey FromRawBytes(const CBytes& rawBytes, KeyAlgo keyAlgo)
    {
        try
        {
            ...
            {
                SPDLOG_ERROR("rawBytes.size(): {} different than expectedPublicKeySize: {}. For keyAlgo: {}",
    ...
    std::string GetAccountHash() const
    {
        ...
        CryptoPP::SecByteBlock algo_bytes = hexDecode(StringUtil::getStringBytesWithoutLength(algo_str));
    ...
    */
    ...
    bool operator<(const Casper::PublicKey& other) const { return ToAccountHex() < other.ToAccountHex(); }

    bool operator==(const Casper::PublicKey& other) const { return ToAccountHex() == other.ToAccountHex(); }

    bool operator!=(const Casper::PublicKey& other) const { return !(*this == other); }
};
