...
struct ExecutableDeployItem
{
    ...
    std::optional<StoredVersionedContractByHash> stored_versioned_contract_by_hash = std::nullopt;
    ...
    ExecutableDeployItem(StoredVersionedContractByHash stored_versioned_contract_by_hash)
        : stored_versioned_contract_by_hash(stored_versioned_contract_by_hash)
    {
    }

    ExecutableDeployItem(StoredVersionedContractByName stored_versioned_contract_by_name)
        : stored_versioned_contract_by_name(stored_versioned_contract_by_name)
    {
    }

    ExecutableDeployItem(TransferDeployItem transfer)
        : transfer(transfer)
    {
    }
};
...
inline void to_json(nlohmann::json& j, const ExecutableDeployItem& p)
{
    ...
    {
        j["StoredVersionedContractByHash"] = p.stored_versioned_contract_by_hash.value();
    }
    else if (p.stored_versioned_contract_by_name.has_value())
    {
        j["StoredVersionedContractByName"] = p.stored_versioned_contract_by_name.value();
    }
...
inline void from_json(const nlohmann::json& j, ExecutableDeployItem& p)
{
    ...
    {
        p.stored_contract_by_hash = j["StoredContractByHash"].get<StoredContractByHash>();
    }
    else if (j.find("StoredContractByName") != j.end())
    {
        p.stored_contract_by_name = j["StoredContractByName"].get<StoredContractByName>();
    }
    else if (j.find("StoredVersionedContractByHash") != j.end())
    {
        p.stored_versioned_contract_by_hash = j["StoredVersionedContractByHash"].get<StoredVersionedContractByHash>();
    }
    else if (j.find("StoredVersionedContractByName") != j.end())
    {
        p.stored_versioned_contract_by_name = j["StoredVersionedContractByName"].get<StoredVersionedContractByName>();
    }
