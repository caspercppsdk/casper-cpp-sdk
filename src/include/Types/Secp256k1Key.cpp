...
std::string Secp256k1Key::getPublicKeyStr()
{
    return public_key_str;
}

std::string Secp256k1Key::getPrivateKeyStr()
{
...
std::string Secp256k1Key::sign(std::string message)
{
    ...
    siglen =
        signer.SignMessage(prng, (const CryptoPP::byte*)&message[0], message.size(), (CryptoPP::byte*)&signature[0]);
    signature.resize(siglen);

    return signature;
}

CryptoPP::SecByteBlock Secp256k1Key::sign(const CryptoPP::SecByteBlock& message)
{
    CryptoPP::AutoSeededRandomPool prng;
    CryptoPP::ECDSA<CryptoPP::ECP, CryptoPP::SHA256>::Signer signer(_private_key);
...
bool Secp256k1Key::verify(std::string message, std::string signature)
{
    ...
    CryptoPP::ECDSA<CryptoPP::ECP, CryptoPP::SHA256>::Verifier verifier(_public_key);

    bool result = verifier.VerifyMessage((const CryptoPP::byte*)&message[0], message.size(),
