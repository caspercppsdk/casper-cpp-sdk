...
using CLTypeRVA =
    boost::make_recursive_variant<CLTypeEnum,                                                     // primitives
...
inline void to_json(nlohmann::json& j, const CLTypeRVA& p)
{
    ...
    if (p.which() == 0)
    {
        ...
        if (p_type == CLTypeEnum::Bool || p_type == CLTypeEnum::I32 || p_type == CLTypeEnum::I64 ||
            p_type == CLTypeEnum::U8 || p_type == CLTypeEnum::U32 || p_type == CLTypeEnum::U64 ||
            p_type == CLTypeEnum::U128 || p_type == CLTypeEnum::U256 || p_type == CLTypeEnum::U512 ||
            p_type == CLTypeEnum::Unit || p_type == CLTypeEnum::String || p_type == CLTypeEnum::Key ||
            p_type == CLTypeEnum::URef || p_type == CLTypeEnum::Any || p_type == CLTypeEnum::PublicKey)
        {
    ...
    else if (p.which() == 2)
    {
        ...
        j["Map"] = {{"key", p_type.begin()->first}, {"value", p_type.begin()->second}};
    }
    /// option, list, result
    else if (p.which() == 3)
    {
        ...
        else if (key_type == "Result")
        {
            ...
            nlohmann::json inner_val = {{"Ok", inner_map["Ok"]}, {"Err", inner_map["Err"]}};
            j = {"Result", inner_val};
        }
    }
