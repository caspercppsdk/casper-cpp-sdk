...
struct CLValue
{
    ...
    CLValue(CBytes bytes, CLType cl_type)
        : cl_type(cl_type)
        , bytes(bytes)
    {
    }
    ...
    CLValue(CBytes bytes, CLTypeRVA cl_type)
        : cl_type(cl_type)
        , bytes(bytes)
    {
    }
    ...
    static CLValue Option(CLValue innerValue)
    {
        ...
        std::copy(innerValue.bytes.begin(), innerValue.bytes.end(), bytes.begin() + 1);
    ...
    static CLValue Option(int32_t innerValue) { return CLValue::Option(CLValue::I32(innerValue)); }

    static CLValue Option(int64_t innerValue) { return CLValue::Option(CLValue::I64(innerValue)); }

    static CLValue Option(uint8_t innerValue) { return CLValue::Option(CLValue::U8(innerValue)); }

    static CLValue Option(uint32_t innerValue) { return CLValue::Option(CLValue::U32(innerValue)); }

    static CLValue Option(uint64_t innerValue) { return CLValue::Option(CLValue::U64(innerValue)); }

    static CLValue Option(std::string innerValue) { return CLValue::Option(CLValue::String(innerValue)); }

    static CLValue Option(Casper::URef innerValue) { return CLValue::Option(CLValue::URef(innerValue)); }
    /*
    ...
    */
    ...
    static CLValue Option(PublicKey innerValue) { return CLValue::Option(CLValue::PublicKey(innerValue)); }

    static CLValue Option(GlobalStateKey innerValue) { return CLValue::Option(CLValue::Key(innerValue)); }

    static CLValue Option(CBytes innerValue) { return CLValue::Option(CLValue::ByteArray(innerValue)); }

    static CLValue Option(std::vector<CLValue>& innerValue) { return CLValue::Option(CLValue::List(innerValue)); }

    static CLValue Option(std::map<CLValue, CLValue> innerValue) { return CLValue::Option(CLValue::Map(innerValue)); }
    ...
    static CLValue OptionNoneTuple2(CLTypeRVA innerTypeInfo1, CLTypeRVA innerTypeInfo2)
    {
        CBytes bytes(1);
    ...
    static CLValue OptionNoneTuple3(CLTypeRVA innerTypeInfo1, CLTypeRVA innerTypeInfo2, CLTypeRVA innerTypeInfo3)
    {
        CBytes bytes(1);
    ...
    static CLValue OptionNone(CLTypeEnum innerType) { return CLValue::OptionNone(CLType(innerType)); }
    ...
    static CLValue List(std::vector<CLValue> values)
    {
        ...
        {
            SPDLOG_ERROR(
                "List size is 0"); // Is it worth to print this info here, when there is same information thrown?
        ...
        for (auto value : values)
        {
            ...
            {
                throw std::runtime_error("All elements in a list must be of the same type");
            }
        }
    ...
    /// </summary>
    static CLValue ByteArray(CBytes bytes) { return CLValue(bytes, CLType(bytes.size()), hexEncode(bytes)); }
    ...
    static CLValue Map(std::map<CLValue, CLValue>& dict)
    {
        ...
        for (auto kv : dict)
        {
            ...
            {
                throw std::runtime_error("All elements in a map must be of the same "
                                         "type");
            }
    ...
    static CLValue Tuple2(CLValue t0, CLValue t1)
    {
        ...
        std::copy(t1.bytes.begin(), t1.bytes.end(), bytes.begin() + t0.bytes.size());
    ...
    static CLValue Tuple3(CLValue t0, CLValue t1, CLValue t2)
    {
        ...
        std::copy(t2.bytes.begin(), t2.bytes.end(), bytes.begin() + t0.bytes.size() + t1.bytes.size());
    ...
    {
        return CLValue(publicKey.GetBytes(), CLType(CLTypeEnum::PublicKey), hexEncode(publicKey.GetBytes()));
    }
