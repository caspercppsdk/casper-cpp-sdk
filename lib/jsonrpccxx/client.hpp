...
class JsonRpcClient {
 public:
  JsonRpcClient(IClientConnector &connector, version v)
      : connector(connector), v(v) {}
  ...
  template <typename T>
  T CallMethod(const id_type &id, const std::string &name,
               const positional_parameter &params) {
  ...
  nlohmann::json CallMethodNamed_JsonRepr(const id_type &id,
                                          const std::string &name,
                                          const named_parameter &params = {}) {
    return call_method(id, name, params).result;
  }
  ...
  void CallNotificationNamed(const std::string &name,
                             const named_parameter &params = {}) {
    call_notification(name, params);
  }
 ...
 private:
    ...
    try {
      ...
      if (has_key(response, "result") && has_key(response, "id")) {
        if (response["id"].type() == json::value_t::string)
          return JsonRpcResponse{response["id"].get<std::string>(),
                                 response["result"].get<json>()};
        else
          return JsonRpcResponse{response["id"].get<int>(),
                                 response["result"].get<json>()};
      }
  ...
  void call_notification(const std::string &name,
