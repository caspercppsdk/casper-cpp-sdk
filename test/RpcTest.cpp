...
{
    return {Casper::PublicKey::FromHexString(pubKey), getTimestampNow(), "30m", 1, "", {}, "casper-test"};
}
...
void chainGetStateRootHash_with_blockHeightTest(void)
{
    ...
    std::string expected_result = "4d180287e6eb3dad5173864e30d7653c01fcdef8bc3ee31db4a0707367154ccf";
...
void chainGetStateRootHash_with_blockHashTest(void)
{
    ...
    std::string block_hash = "9511adf5ed36ccba48d71840fd558d4397c9eb0470d1e25711b5419632a6f55a";
    std::string result = client.GetStateRootHash(block_hash).state_root_hash;
    std::string expected_result = "9aa3c10d4db2e02adb464458c7a09d1df2ed551be02d2c7bbdbe847d3731e84f";
...
{
    Client client(CASPER_TEST_ADDRESS);
    std::string deploy_hash = "8e535d2baed76141ab47fd93b04dd61f65a07893b7c950022978a2b29628edd7";
    ...
    TEST_ASSERT(iequals(deploy_info.at("hash"), "8e535d2baed76141ab47fd93b04dd61f65a07893b7c950022978a2b29628edd7"));
    ...
    TEST_ASSERT(
        iequals(deploy_header.at("body_hash"), "11f5a10f791fd6ac8b12d52298b7d1db7bd91e8c15b5d1330fd16d792257693c"));
...
void chainGetBlockTransfers_with_blockHashTest(void)
{
    ...
    std::string block_hash = "35f86b6ab5e13b823daee5d23f3373f6b35048e0b0ea993adfadc5ba8ee7aae5";
    ...
    std::string expected_deploy_hash = "8e535d2baed76141ab47fd93b04dd61f65a07893b7c950022978a2b29628edd7";
    std::string expected_from = "account-hash-"
                                "308d2a0eCF66bDAcAC5Cf6184C732D83DCeB48A859169e5680FE17cF32Bb974F";
    uint512_t expected_gas = u512FromDec("0");
    std::string expected_source = "uref-5ce1d189e8ccafdd5a959088ffd870f54b29bd5afeb05950dddcc12ec7dcbe90-"
                                  "007";
    std::string expected_target = "uref-c9733355d61aa2a36721d9d1081eebcfe5dde94f82386b3d75163fee894d292a-"
                                  "007";
    ...
    auto test_transfer = std::find_if(transfers.begin(), transfers.end(),
                                      [&](const Transfer& x) { return iequals(x.deploy_hash, expected_deploy_hash); });
...
void chainGetBlock_with_blockHashTest(void)
{
    ...
    std::string block_hash = "c512989d5a850bd9cc32fbf54a2046ee422f8a14e1e9003189a7cf987340d073";
    ...
    TEST_ASSERT(iequals(current_block.hash, "c512989d5a850bd9cc32fbf54a2046ee422f8a14e1e9003189a7cf987340d073"));

    // block header
    TEST_ASSERT(
        iequals(current_block.header.parent_hash, "45a089f5d0575227721526d59b24ab6e860b560d709d9b1760b4378a363c142b"));

    TEST_ASSERT(iequals(current_block.header.state_root_hash,
                        "116c27db7fbc6ea1c8dcd41b21f655e22913478962a27c746ac68796644f528c"));

    TEST_ASSERT(
        iequals(current_block.header.body_hash, "9b79475df86baafbf93ac59a5b11a462157383fe0101ee235cd8aee58daf14cd"));

    TEST_ASSERT(iequals(current_block.header.accumulated_seed,
    ...
    TEST_ASSERT(iequals(current_block.body.proposer.ToString(),
...
void chainGetEraInfoBySwitchBlock_with_blockHashTest(void)
{
    ...
    GetEraInfoResult result =
        client.GetEraInfoBySwitchBlock("d2077716e5b8796723c5720237239720f54e6ada54e3357f2c4896f2a51a6d8f");
    ...
    TEST_ASSERT(result.era_summary.value().stored_value.era_info.value().seigniorage_allocations.size() > 0);
    ...
    for (int i = 0; i < result.era_summary.value().stored_value.era_info.value().seigniorage_allocations.size(); i++)
    {
        ...
        bool is_delegator =
            result.era_summary.value().stored_value.era_info.value().seigniorage_allocations[i].is_delegator;
        ...
        if (is_delegator == true && delegator_cnt == 0)
        {
            ...
            TEST_ASSERT(result.era_summary.value().stored_value.era_info.value().seigniorage_allocations[i].amount >=
                        0);
        }
...
void stateGetItem_with_keyTest(void)
{
    ...
    TEST_ASSERT(
        iequals(current_transfer.deploy_hash, "8e535d2baed76141ab47fd93b04dd61f65a07893b7c950022978a2b29628edd7"));

    TEST_ASSERT(iequals(current_transfer.from.ToString(),
                        "account-hash-"
                        "308d2a0eCF66bDAcAC5Cf6184C732D83DCeB48A859169e5680FE17cF32Bb974F"));

    TEST_ASSERT(iequals(current_transfer.source.ToString(),
                        "uref-5ce1d189e8ccafdd5a959088ffd870f54b29bd5afeb05950dddcc12ec7dcbe90-"
                        "007"));

    TEST_ASSERT(iequals(current_transfer.target.ToString(),
...
void stateGetDictionaryItem_with_keyTest(void)
{
    ...
    TEST_ASSERT(
        iequals(dictionaryItemResult.at("stored_value").at("CLValue").at("bytes"), "090000006162635F76616c7565"));

    TEST_ASSERT(iequals(dictionaryItemResult.at("stored_value").at("CLValue").at("cl_type"), "String"));
}
...
void stateGetBalance_with_urefTest(void)
{
    ...
    std::string purse_uref = "uref-54fd72455872082a254b0160e94a86245acd0c441f526688bda1261d0969057a-"
                             "007";
...
void stateGetAuctionInfo_with_blockHashTest(void)
{
    ...
    TEST_ASSERT(iequals(auction_result.auction_state.state_root_hash,
    ...
    TEST_ASSERT(auction_result.auction_state.era_validators[0].validator_weights.size() > 0);
    TEST_ASSERT(auction_result.auction_state.era_validators[0].validator_weights[0].public_key.ToString() != "");
    TEST_ASSERT(auction_result.auction_state.era_validators[0].validator_weights[0].weight > 0);

    TEST_ASSERT(auction_result.auction_state.bids.size() > 0);
    TEST_ASSERT(auction_result.auction_state.bids[0].public_key.ToString() != "");

    TEST_ASSERT(auction_result.auction_state.bids[0].bid.validator_public_key.ToString() != "");
    TEST_ASSERT(auction_result.auction_state.bids[0].bid.bonding_purse.ToString() != "");
...
void PutDeploy_StoredContractByHash_Test(void)
{
    ...
    StoredContractByHash scbh(
        "e3523602448b6085b861890b1c214181e2c1a7bdd2b23424b1941d1301256517", "unlock_cspr",
        {NamedArg("receipient_publickey", CLValue::String("01f28f169dad17315a03d15e16aa93d5342303ae11f"
                                                          "15c68aadfdab3df31b0fcbf")),
         NamedArg("bsc_transaction_hash", CLValue::String("0xd6e7f5aa561e069c385d0f63a3c8dc5501f63d3616c3"
...
{
    Client client(CASPER_TEST_ADDRESS);
    std::string block_hash = "a55471527f5b3aa75f8e9c26b781369623d24ada613e09ed341bb56968b7ede1";
